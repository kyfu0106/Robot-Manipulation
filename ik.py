...
def your_ik(robot, new_pose : list or tuple or np.ndarray, 
                max_iters : int=4000, stop_thresh : float=.001):
    # max_iter=1000, stop_thresh=.001
    ...
    for i in range(max_iters):
        current_pose, jacobian = your_fk(robot, DH_params, tmp_q)   # 7d, 6x7
        #print(f'norm:{np.linalg.norm(new_pose - current_pose)}')
        if np.linalg.norm(new_pose - current_pose) < stop_thresh:
            break
        m_new_pose = get_matrix_from_pose(new_pose)
        m_current_pose = get_matrix_from_pose(current_pose)
        m_dx = m_new_pose @ np.linalg.pinv(m_current_pose)
        dx = get_pose_from_matrix(m_dx, 6)  # 6d
        dt = step_size * np.linalg.pinv(jacobian) @ dx  # 7d
        tmp_q += dt
 
        for j in range(len(tmp_q)):
            if tmp_q[j] >= joint_limits[j, 1]:
                tmp_q[j] = joint_limits[j, 1] - 0.0003 
            elif tmp_q[j] <= joint_limits[j, 0]:
                tmp_q[j] = joint_limits[j, 0] + 0.0003
