...
def your_fk(robot, DH_params : dict, q : list or tuple or np.ndarray) -> np.ndarray:
    ...
    for i, p in enumerate(DH_params):
        ...
        T = np.array([[np.cos(q[i]), -np.sin(q[i]), 0, a],
                        [np.sin(q[i])*np.cos(alpha), np.cos(q[i])*np.cos(alpha), -np.sin(alpha), -d*np.sin(alpha)],
                        [np.sin(q[i])*np.sin(alpha), np.cos(q[i])*np.sin(alpha), np.cos(alpha), d*np.cos(alpha)],
                        [0, 0, 0, 1]])
        A = A @ T
